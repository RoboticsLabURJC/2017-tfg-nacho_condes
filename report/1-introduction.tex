...
\begin{itemize}
	\item \texttt{Detection Suite}\footnote{\url{http://jderobot.org/DetectionSuite}}\footnote{\url{https://github.com/JdeRobot/dl-DetectionSuite}}: it is a C++ application, suitable to load/benchmark \textbf{detection} models, against different databases. It is also, through a Python$\rightarrow$C++ interface, to load TensorFlow/Keras models as well.
	
	\item Final project of David Pascual \cite{dpascualhe} and Nuria Oyaga \cite{noyaga}: a further study of Deep Learning, applied on Python (Keras and Caffe frameworks, respectively) to \textbf{digits classification} implementing a CNN as it has been seen.
